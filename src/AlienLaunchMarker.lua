...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        elseif love.mouse.wasReleased(1) and self.aiming then
            ...
            self.aliens[#self.aliens].body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10)

            -- make the alien pretty bouncy
            self.aliens[#self.aliens].fixture:setRestitution(0.4)
            self.aliens[#self.aliens].body:setAngularDamping(1)
    ...
    elseif self.launched and not self.hasCollided and not self.hasCreatedNew then
        if love.keyboard.isDown("space") then
            self.hasCreatedNew = true
            self:createNewPlayer()
        end
    end
end

function AlienLaunchMarker:createNewPlayer()
    table.insert(self.aliens, Alien(self.world, 'round', self.aliens[1].body:getX() - 2, self.aliens[1].body:getY() - 2, 'Player'))
    self.aliens[#self.aliens].body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10)
    self.aliens[#self.aliens].fixture:setRestitution(0.4)
    self.aliens[#self.aliens].body:setAngularDamping(1)
    table.insert(self.aliens, Alien(self.world, 'round', self.aliens[1].body:getX() + 5, self.aliens[1].body:getY() + 4, 'Player'))
    self.aliens[#self.aliens].body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10)
    self.aliens[#self.aliens].fixture:setRestitution(0.4)
    self.aliens[#self.aliens].body:setAngularDamping(1)
end
